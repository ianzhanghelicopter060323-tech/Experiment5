...
3. 由 `Book` 派生出一个类 `TextBook`，采用 **private 继承**。

4. 在派生类 `TextBook` 中增加一个数据成员：
   - `subject`（整型，表示学科编号）

5. 在 `TextBook` 中定义构造函数，对基类部分和新增成员进行初始化。

6. 在 `TextBook` 中重新定义一个 `show()` 成员函数，要求：
   - 输出 `subject`
   - 再调用基类的 `show()` 函数输出基类部分信息

7. 在 `TextBook` 中重新定义一个 `getPrice()` 成员函数，其内部调用基类的 `getPrice()`，使类外仍然能够通过 `TextBook` 对象获取价格。

8. 在 `main()` 函数中定义一个 `TextBook` 对象，测试：
   - 调用 `TextBook::show()`
   - 调用重新公开后的 `getPrice()`

9. 思考并说明：
   - 为什么派生类中重新定义 `show()` 后，基类的同名 `show()` 会被隐藏
   - 为什么可以通过 `Book::show()` 的方式调用基类版本
   - 为什么在 `private` 继承下，需要在派生类中重新包一层 `getPrice()` 才能让类外访问

## 目的

通过本题掌握：

- 派生类与基类的同名成员函数之间的隐藏关系
- 使用 `基类名::函数名` 调用基类同名函数的方法
- 在 `private` 继承条件下，如何通过派生类重新公开基类接口